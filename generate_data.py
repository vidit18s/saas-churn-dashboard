...
print(f"Active: {len(df[df.status=='Active'])}  |  Churned: {len(df[df.status=='Churned'])}")
print(f"Total ARR: £{df[df.status=='Active']['arr'].sum():,.0f}")
print(f"Avg MRR: £{df['mrr'].mean():,.0f}")
print(df.groupby('segment')['arr'].mean().round(0))